# Implementation Plan: Real-Time Sign Language Interpreter

## Overview

This plan implements a real-time ASL/BSL sign language interpreter in incremental stages. The backend pipeline (landmark extraction, ML models, ONNX export, FastAPI WebSocket server) is complete. Remaining work covers the browser-side inference engine, post-processing utilities, output buffer, UI, and WebRTC embedding — all in TypeScript/JavaScript.
...
- [x] 2. Implement landmark normalization utilities
  ...
  - [x] 2.5 Implement JavaScript landmark normalizer
    - Write `browser/landmarks.ts` with `normalizeLandmarks()`, `buildTwoHandVector()` — mirrors Python logic exactly (same wrist-origin translation, same index-MCP scale)
    - _Requirements: 1.3, 4.1_

  - [x]* 2.6 Write property test for JavaScript landmark normalization
    - **Property 4: Word-level feature vector has correct shape** (fast-check, `tests/js/landmarks.test.ts`)
    - **Validates: Requirements 3.1**
...
- [x] 6. Checkpoint — ensure all Python and JavaScript tests pass
  - All Python property/unit tests and JS landmark tests pass.

- [x] 7. Implement FastAPI WebSocket server
  - [x] 7.1 Implement server configuration and ONNX session registry
  ...
  - [x] 7.2 Implement WebSocket fingerspelling endpoint
  ...
  - [x]* 7.3 Write property test for WebSocket payload validation
    - **Property 14: WebSocket payload validation rejects wrong-length arrays**
    - **Validates: Requirements 8.2**

  - [x]* 7.4 Write property test for WebSocket response schema
    - **Property 15: WebSocket response contains all required fields**
    - **Validates: Requirements 8.3**

  - [x] 7.5 Implement WebSocket word-level endpoint
  ...
  - [x] 7.6 Write property test for landmark JSON round-trip
    - **Property 16: Landmark JSON serialization round-trip**
    - **Validates: Requirements 8.5**

- [x] 8. Implement browser inference engine
  - [x] 8.1 Implement OrtEngine class
  ...
  - [x] 8.2 Write property test for OrtEngine round-trip (fast-check)
    - **Property 12: ONNX model serialization round-trip** (browser path)
    - **Validates: Requirements 7.3**

  - [x] 8.3 Implement browser PredictionGate
  ...
  - [x] 8.4 Write property test for browser PredictionGate (fast-check)
    - **Property 3: PredictionGate commits exactly once per stable run**
    - **Validates: Requirements 2.4, 9.5, 9.6**
...
- [-] 9. Implement post-processing and output buffer
  ...
  - [x] 9.2 Write property test for OutputBuffer append invariant (fast-check)
    - **Property 7: Transcript append invariant**
    - **Validates: Requirements 5.3**

  - [x] 9.3 Write property test for OutputBuffer backspace invariant (fast-check)
    - **Property 11: Backspace decrements committed length**
    - **Validates: Requirements 5.10**

  - [x] 9.4 Implement fingerspelling collapse and spell-check
  ...
  - [x] 9.5 Write property test for fingerspelling collapse (fast-check)
    - **Property 9: Fingerspelling collapse correctness**
    - **Validates: Requirements 5.7**

  - [x] 9.6 Write property test for spell-check idempotence (fast-check)
    - **Property 10: Spell-check idempotence on dictionary words**
    - **Validates: Requirements 5.8**

  - [x] 9.7 Implement gloss-to-English post-processor
...
- Tasks 1–7 are complete; remaining work is the browser-side pipeline (tasks 8–14)
- Property tests validate the universal correctness properties defined in design.md
- Unit tests validate specific examples and edge cases not easily covered by PBT generators
...
{
  "waves": [
    { "id": 0, "tasks": ["8.1", "9.1", "10.1"] },
    { "id": 1, "tasks": ["8.2", "8.3", "9.2", "9.3", "9.4", "10.2", "10.3"] },
    { "id": 2, "tasks": ["8.4", "9.5", "9.6", "9.7", "10.4", "10.5", "11.1"] },
    { "id": 3, "tasks": ["9.8", "9.9", "11.2"] },
    { "id": 4, "tasks": ["9.10", "12"] },
    { "id": 5, "tasks": ["13.1"] },
    { "id": 6, "tasks": ["14"] }
  ]
